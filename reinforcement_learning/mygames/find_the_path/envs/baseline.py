...
class CEnv(gym.Env):
    ...
    def _build_minimap(self,size = None):
        ...
        minimap = np.zeros(size_with_wall,np.int32)
        cx,cy  = size[1]/2., size[0]/2.
        ally,allx = np.arange(0,size[0]), np.arange(0,size[1])
        yy,xx = np.meshgrid(allx,ally,indexing="xy")
        minimap = np.sqrt((yy - cy)**2 + (xx - cx)**2 )
        minimap = minimap.max() - minimap
        return minimap, (1,1)
