...
config['task_crnn']['train_tfrec_file'] = os.path.join(config['project_root'], config['task_crnn']['train_tfrec_file'])

config['task_crnn']['test_tfrec_file'] = os.path.join(config['project_root'], config['task_crnn']['test_tfrec_file'])

if config_captcha['max_text_length'] != config_captcha['min_text_length']:
    config_charset['chars'] += [' '] #最后一个作为blank
...
def convert_to_tfrecord(split):
    if split == 'train':
        image_usage_req = 1
        outfile = config['task_crnn']['train_tfrec_file']
    else:
        image_usage_req = 0
        outfile = config['task_crnn']['test_tfrec_file']
    ...
    input_width, input_height = config['task_crnn']['input_width'],config['task_crnn']['input_height']
...
def test_tfrd(tfrec_file):
    ...
    def _parse_function(example_proto):
        features = {INPUT_1: tf.io.FixedLenFeature((config['task_crnn']['input_height'],config['task_crnn']['input_width'],3), tf.int64),
                    INPUT_2: tf.io.VarLenFeature(dtype=tf.int64)}
        ...
        #sparse编码转dense编码
        parsed_features[INPUT_2] = tf.sparse.to_dense(parsed_features[INPUT_2])
        #parsed_features[INPUT_2] = tf.cast(parsed_features[INPUT_2],tf.int64)
...
if os.path.exists(config['task_crnn']['train_tfrec_file']) and os.path.exists(config['task_crnn']['test_tfrec_file']):
    print("reuse train.tfrecord and val.tfrecord")
    test_tfrd(config['task_crnn']['test_tfrec_file'])
    test_tfrd(config['task_crnn']['train_tfrec_file'])
